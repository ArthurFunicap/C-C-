...
void alterar(FILE* arq, NO* tabelaHashing[]) {
	...
	if(retorno == -1){
		...
		printf("Deseja alterar a marca do carro? (1 - Sim/2 - Não)\n");
		scanf("%d", &alterar);
		if(alterar == 1){
		    printf("Digite a marca do carro: ");
		    fgets(carroAlterar->marca, 15, stdin);
		}
		
		printf("Deseja alterar o modelo do carro? (1 - Sim/2 - Não)\n");
		scanf("%d", &alterar);
		if(alterar == 1){
		    printf("Digite o modelo do carro: ");
		    fgets(carroAlterar->modelo, 15, stdin);
		}
		
		printf("Deseja alterar a cor do carro? (1 - Sim/2 - Não)\n");
		scanf("%d", &alterar);
		if(alterar == 1){
		    printf("Digite a cor do carro: ");
		    fgets(carroAlterar->cor, 15, stdin);
		}
		
		fseek(arq, retorno * sizeof(CARRO), SEEK_SET);
		fwrite(carroAlterar, sizeof(CARRO), 1, arq);
		
		printf("Valor(es) alterado(s)!\n");
	}
}
